...
L2000:
    wcprintf("GDT>");
/* 						!OUTPUT PROMPT. */
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
    cmd[0] = ' ';
    cmd[1] = ' ';
    sscanf(buf, "%2s", cmd);
...
/* 						!COL HDRS */
    ...
    for (i = j; i <= i__1; ++i) {
	more_output(NULL);
	wcprintf("%3d%6d%6d%6d%4d%7d%7d%4d%4d%6d%6d %4d%4d%4d%6d\n",
...
/* 						!COMPUTE END OF LINE. */
	...
	wcprintf("%3d-%3d  ", i, l);
	for (l1 = i; l1 <= l; ++l1)
	    wcprintf("%7d", exits_1.travel[l1 - 1]);
	wcprintf("\n");
/* L14100: */
    }
    goto L2000;
...
L15000:
    more_output(NULL);
    wcprintf("THFPOS= %d, THFFLG= %c, THFACT= %c\n",
	   hack_1.thfpos, hack_1.thfflg ? 'T' : 'F',
	   hack_1.thfact ? 'T' : 'F');
    more_output(NULL);
    wcprintf("SWDACT= %c, SWDSTA= %d\n", hack_1.swdact ? 'T' : 'F',
	   hack_1.swdsta);
    goto L2000;
...
L19000:
    more_output(NULL);
    wcprintf("Parse vector= %6d %6d %6d      %c %6d\n",
	   prsvec_1.prsa, prsvec_1.prso, prsvec_1.prsi,
	   prsvec_1.prswon ? 'T' : 'F', prsvec_1.prscon);
    more_output(NULL);
    wcprintf("Play vector=  %6d %6d      %c\n", play_1.winner, play_1.here,
	    play_1.telflg ? 'T' : 'F');
    more_output(NULL);
    wcprintf("State vector= %6d %6d %6d %6d %6d %6d %6d %6d %6d\n",
...
/* 						!ENTRY NO VALID? */
    wcprintf("Old= %c      New= ", flags[j - 1] ? 'T' : 'F');
/* 						!TYPE OLD, GET NEW. */
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!INDICES VALID? */
    wcprintf("Old = %6d      New = ", eqr[j + k * 200 - 201]);
/* 						!TYPE OLD, GET NEW. */
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!INDICES VALID? */
    wcprintf("Old = %6d      New = ", eqo[j + k * 200 - 201]);
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!INDICES VALID? */
    wcprintf("Old = %6d      New = ", eqa[j + (k << 2) - 5]);
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!FLAGS ENTRY? */
    wcprintf("Old = %6d      New = ", eqc[j + k * 25 - 26]);
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!ENTRY NO VALID? */
    wcprintf("Old= %6d     New= ", exits_1.travel[j - 1]);
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!INDICES VALID? */
    wcprintf("Old = %6d      New= ", eqv[j + (k << 2) - 5]);
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!VALID ENTRY? */
    wcprintf("Old= %6d      New= ", switch_[j - 1]);
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
L43000:
    wcprintf("Old= %6d      New= ", play_1.here);
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
L45000:
    wcprintf("Old= %6d      New= ", debug_1.prsflg);
/* 						!TYPE OLD, GET NEW. */
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
...
/* 						!DISPLAY PUZZLE */
	...
	wcprintf(" ");
	for (j = i; j <= i + 7; ++j)
	     wcprintf("%3d", puzzle_1.cpvec[j - 1]);
	wcprintf("\n");
/* L46100: */
    }
    goto L2000;
...
/* 						!VALID ENTRY? */
    wcprintf("Old= %6d      New= ", puzzle_1.cpvec[j - 1]);
/* 						!OUTPUT OLD, */
    (void) wcfflush(stdout);
    (void) wcfgets(buf, sizeof buf, stdin);
    more_input();
